...
static int extract_matrix_cmp(
        const extract_matrix_t* lhs,
        const extract_matrix_t* rhs
        )
{
    int ret;
...
+/-1. */
static int extract_matrix_cmp4(
        const extract_matrix_t* lhs,
        const extract_matrix_t* rhs
        )
{
        ...
        )
{
    ...
    extract_outfx("Unzipping template document '%s' to tempdir: %s",
    ...
    if (!f) {
        extract_outf("error: Failed to open .docx for writing: %s",
                word_document_xml);
        goto end;
    }
    if (0
...
/* Appends new extract_char_t to an extract_span_t with .ucs=c and all other
fields zeroed. */
static int extract_span_append_c(extract_span_t* span, int c)
{
    extract_char_t* items = realloc(
            span->chars,
...
static const char* extract_paragraph_string(extract_paragraph_t* paragraph)
{
    ...
    if (paragraph->lines_num) {
        ...
        if (paragraph->lines_num > 1) {
            ...
            extract_string_cat(
                    &ret,
                    extract_line_string2(paragraph->lines[paragraph->lines_num-1])
                    );
        }
    }
    return ret.chars;
...
static extract_line_t* extract_paragraph_line_first(
        const extract_paragraph_t* paragraph
        )
{
...
static extract_line_t* extract_paragraph_line_last(
        const extract_paragraph_t* paragraph
        )
{
    assert(paragraph->lines_num);
        ...
        )
{
    ...
    if (extract_line_span_first(a)->wmode != extract_line_span_first(b)->wmode) {
        return 0;
    }
    if (extract_matrix_cmp4(
            &extract_line_span_first(a)->ctm,
            &extract_line_span_first(b)->ctm
            )) {
        if (verbose) {
...
/* Creates representation of extract_span_t's that consists of a list of
extract_line_t's, with each extract_line_t containins pointers to a list of
extract_span_t's.
...
On entry:
    ...
    <spans> points to array of <spans_num> extract_span_t*'s, each pointing to
    an extract_span_t.

On exit:
    If we succeed, we return 0, with *o_lines pointing to array of *o_lines_num
    extract_line_t*'s, each pointing to an extract_line_t.
        ...
        )
{
    ...
    /* Make an extract_line_t for each span. Then we will join some of these
    extract_line_t's together before returning. */
    ...
    for (a=0; a<lines_num; ++a) {
            ...
            */
            ...
            if (fabs(angle_a_b - angle_a) * 180 / g_pi <= angle_tolerance_deg) {
                ...
                float adv = spans_adv(
                        ...
                        extract_span_char_first(span_b)
                        );
        ...
        if (nearest_line) {
            /* extract_line_a and nearest_line are aligned so we can move
            extract_line_b's spans on to the end of extract_line_a. */
            b = nearest_line_b;
            ...
            if (1
                ...
                if (insert_space) {
                    ...
                    extract_char_t* p = realloc(
                            span_a->chars,
                            (span_a->chars_num + 1) * sizeof(extract_char_t)
                            );
                    if (!p) goto end;
                    span_a->chars = p;
...
/* Returns max font size of all extract_span_t's in an extract_line_t. */
...
*/
static float extract_line_distance(
        float ax,
        float ay,
        float bx,
        float by,
...
On entry:
    ...
    <lines> points to array of <lines_num> extract_line_t*'s, each pointing to
    a extract_line_t.

On exit:
    On sucess, returns zero, *o_paragraphs points to array of *o_paragraphs_num
    extract_paragraph_t*'s, each pointing to an extract_paragraph_t. In the
    array, extract_paragraph_t's with same angle are sorted.
        ...
        )
{
    ...
    /* Start off with an extract_paragraph_t for each extract_line_t. */
    ...
    for (a=0; a<paragraphs_num; ++a) {
        ...
        /* Look for nearest extract_paragraph_t that could be appended to
        paragraph_a. */
        int b;
        ...
        if (nearest_paragraph) {
            ...
            float line_b_size = extract_line_font_size_max(
                    extract_paragraph_line_first(nearest_paragraph)
                    );
            if (nearest_paragraph_distance < 1.5 * line_b_size) {
                if (verbose) {
                    ...
                    extract_outf("paragraph_a ctm=%s",
                            extract_matrix_string(&paragraph_a->lines[0]->spans[0]->ctm)
                            );
                    extract_outf("paragraph_a trm=%s",
                ...
                extract_line_t** l = realloc(
                        paragraph_a->lines,
                        sizeof(extract_line_t*) * a_lines_num_new
                        );
                if (!l) goto end;
                paragraph_a->lines = l;
                int i;
                ...
                extract_outfx(
                        "Not joining paragraphs. nearest_paragraph_distance=%lf extract_line_b_size=%lf",
                        nearest_paragraph_distance,
                        extract_line_b_size
                        );
            }
    ...
    paragraphs_num = to;
    extract_paragraph_t** p = realloc(
            paragraphs,
            sizeof(extract_paragraph_t*) * paragraphs_num
            );
    /* Should always succeed because we're not increasing allocation size. */
    assert(p);
    ...
    */
    qsort(
            paragraphs,
            paragraphs_num,
            sizeof(extract_paragraph_t*), paragraphs_cmp
            );
...
/* Appends new empty extract_span_ to an extract_page_t; returns NULL with
errno set on error. */
...
/* Appends new empty extract_page_t to an extract_document_t; returns NULL with errno set on
error. */
static extract_page_t* extract_document_page_append(
        ...
        )
{
    ...
    page->paragraphs = NULL;
    page->paragraphs_num = 0;
    extract_page_t** pages = realloc(
            document->pages,
            sizeof(extract_page_t*) * (document->pages_num + 1)
            );
    if (!pages) {
        free(page);
        return NULL;
...
Looks at last two extract_char_t's in last extract_span_t of <page>, and either
leaves unchanged, or removes space in last-but-one position, or moves last
extract_char_t into a new extract_span_t. */
static int extract_page_span_end_clean(extract_page_t* page)
{
    ...
    float font_size = extract_matrix_expansion(span->trm)
    ...
    if (span->chars_num >= 2 && span->chars[span->chars_num-2].ucs == ' ') {
        ...
        if (remove_penultimate_space) {
            ...
            reason. */
            extract_outfx("removing space before final char in: %s",
        ...
        )
{
    ...
    */
    for(;;) {
        ...
        for(;;) {
            ...
            for(;;) {
                ...
                if (autosplit && char_pre_y - offset_y != 0) {
                    extract_outfx("autosplit: extract_char_pre_y=%f offset_y=%f",
                            extract_char_pre_y, offset_y);
        ...
        extract_outf("page=%i page->num_spans=%i",
...
{
    float font_size = extract_matrix_expansion(*trm)
