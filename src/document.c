...
content_dump_span_aux(const span_t *span, int depth)
{
    ...
    if (span->type != EXTRACT_CLASSIFY_UNKNOWN)
    {
        const char *s = "?????";
        space_prefix(depth);
        printf("      type=");
        ...
        default:
            break;
        case EXTRACT_CLASSIFY_TEXT:
            s = "text";
            break;
        case EXTRACT_CLASSIFY_TITLE:
            s = "title";
            break;
        case EXTRACT_CLASSIFY_LIST:
            s = "list";
            break;
        case EXTRACT_CLASSIFY_TABLE:
            s = "table";
            break;
        case EXTRACT_CLASSIFY_FIGURE:
            s = "figure";
            break;
        }
        printf("\"%s\"\n", s);
