...
static content_root_t *
content_parent(content_root_t *content)
{
    content_t *parent = content->parent;
    content_t *c = parent;
    if (parent == NULL)
        return NULL;
    while (c->type != content_root)
    {
        assert(c->next != parent);
        c = c->next;
    }
    return (content_root_t *)c;
}
...
static rect_t
extract_content_rect(content_t *node)
{
    content_iterator  it;
    content_t        *content;
    ...
    case content_root:
        break;
    case content_line:
        sub = &((line_t *)node)->content;
        break;
    case content_paragraph:
        sub = &((paragraph_t *)node)->content;
        break;
    case content_image:
        break;
    case content_table:
    ...
    case content_block:
        break;
    case content_span:
        rect = extract_span_rect((span_t *)node);
        break;
    }
    if (sub)
        for (content = content_iterator_init(&it, sub); content != NULL; content = content_iterator_next(&it))
        {
            rect = extract_rect_union(rect, extract_content_rect(content));
        }
...
static void
map_classify(content_root_t   *root,
             extract_struct_t *structure,
             double            x0,
             double            y0,
             double            x1,
             double            y1)
...
int
extract_classify_region(extract_t *extract,
                        double     x0,
                        double     y0,
                        double     x1,
                        double     y1)
