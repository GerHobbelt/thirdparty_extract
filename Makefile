...
%.pdf-test-mu: %.pdf $(exe)
	...
	./$(exe) -i test/$<.mu-raw.intermediate.xml --o-content test/$<.mu-raw.content.xml -o test/$<.mu-raw.docx -p 1 -t template.docx
...
%.pdf-test-mu-as: %.pdf $(exe)
	...
	../mupdf/build/debug/mutool draw -F xmltext -o test/$<.mu-raw.intermediate.xml $<
	@echo == Generating output.
	./$(exe) --autosplit 1 -i test/$<.mu-raw.intermediate.xml --o-content test/$<.mu-raw.as.content.xml -o test/$<.mu-raw.as.docx -p 1 -t template.docx
	@echo == Comparing output with reference output.
...
%.pdf-test2-mu: %.pdf $(exe)
	./$(exe) -i test/$<.mu-raw.intermediate2.xml -o test/$<.mu-raw2.docx -p 1 -t template.docx

%.pdf-test4-mu: %.pdf $(exe)
	./$(exe) -i test/$<.mu-raw.intermediate4.xml --scale 0.0 -c test/$<.mu-raw4.content.xml -o test/$<.mu-raw4.docx -p 1 -t template.docx 2>&1 | tee test/$<.mu-raw4.out
%.pdf-test4as-mu: %.pdf $(exe)
	./$(exe) -i test/$<.mu-raw.intermediate4.xml --scale 1.0 -c test/$<.mu-raw4as.content.xml -o test/$<.mu-raw4as.docx -p 1 -t template.docx --autosplit 0 2>&1 | tee test/$<.mu-raw4as.out
